...
def main():
    ...
    parser.add_argument('--task', default='Cyst', help='task or dataset name')
    ...
    parser.add_argument('--batch_size', type=int, default=16, help='batch_size per gpu') # SAMed is 12 bs with 2n_gpu and lr is 0.005
    ...
    model.to(device)
    if opt.pre_trained:
        checkpoint = torch.load(opt.load_path0)
        new_state_dict = {}
    ...
    best_dice, best_model_state = 0.0, None
    ...
    for epoch in range(opt.epochs):
        ...
        print('epoch [{}/{}], train loss:{:.4f}'.format(epoch, opt.epochs, avg_train_loss))
        if args.keep_log:
    ...
    plt.savefig(os.path.join(opt.save_path, 'loss_curve.png'))
    ...
    plt.savefig(os.path.join(opt.save_path, 'dice_score_curve.png'))
