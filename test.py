...
def main():
    ...
    parser.add_argument('--batch_size', type=int, default=16, help='batch_size per gpu') # 8 # SAMed is 12 bs with 2n_gpu and lr is 0.005
    ...
    model.load_state_dict(new_state_dict, strict=False)#修改了状态字典
...
#  ========================================================================= begin to evaluate the model ============================================================================
    ...
    if opt.mode == "train":
        ...
        mean_dice, mean_hdis, mean_iou, mean_acc, mean_se, mean_sp, mean_mcc, std_dice, std_hdis, std_iou, std_acc, std_se, std_sp, std_mcc = get_eval(valloader, model, criterion=criterion, opt=opt, args=args)
        print("dataset:" + args.task + " -----------model name: "+ args.modelname)
        print("task", args.task, "checkpoints:", opt.load_path)
        print(mean_dice[1:], mean_hdis[1:], mean_iou[1:], mean_acc[1:], mean_se[1:], mean_sp[1:], mean_mcc[1:])
        print(std_dice[1:], std_hdis[1:], std_iou[1:], std_acc[1:], std_se[1:], std_sp[1:], std_mcc[1:])
        with open("experiments.txt", "a+") as file:
            file.write(args.task + " " + args.modelname + "\n" )
            file.write("mean_dice:" + '%.2f'%(mean_dice[1]) + "±" + '%.2f'%std_dice[1] + "\n")
            file.write("mean_hd95:" + '%.2f'%mean_hdis[1] + "±" + '%.2f'%std_hdis[1] + "\n")
            file.write("mean_iou:" + '%.2f'%(mean_iou[1]) + "±" + '%.2f'%std_iou[1] + "\n")
            file.write("mean_acc:" + '%.2f'%(mean_acc[1]) + "±" + '%.2f'%std_acc[1] + "\n")
            file.write("mean_recall:" + '%.2f'%(mean_se[1]) + "±" + '%.2f'%std_se[1] + "\n")
            file.write("mean_mcc:" + '%.2f'%(mean_mcc[1]) + "±" + '%.2f'%std_mcc[1] + "\n")
