...
def eval_mask_slice2(valloader, model, criterion, opt, args):
    ...
    ious, accs, ses, sps, mccs = np.zeros((max_slice_number, opt.classes)), np.zeros((max_slice_number, opt.classes)), np.zeros((max_slice_number, opt.classes)), np.zeros((max_slice_number, opt.classes)), np.zeros((max_slice_number, opt.classes))
    eval_number = 0
    sum_time = 0
    ...
    for batch_idx, (datapack) in enumerate(valloader):
        ...
        if args.gen_cam:  
            for j in range(imgs.shape[0]):
                ...
                cv2.imwrite(os.path.join(cam_dir, f"{base_name}_cam.png"), grayscale_cam * 255)
                cv2.imwrite(os.path.join(cam_dir, f"{base_name}_overlay.png"), 
                           show_cam_on_image(img_np, grayscale_cam, use_rgb=False))
        ...
        for j in range(0, b):
            ...
            gt_flat = gt[j].flatten()
            pred_flat = (seg[j].flatten() > 0.5).astype(int)
            mccs[eval_number+j, 1] += matthews_corrcoef(gt_flat, pred_flat)
            del pred_i, gt_i
    ...
    if opt.mode == "train":
        ...
        mcc_mean = np.mean(mccs*100, axis=0)
        mcc_std = np.std(mccs*100, axis=0)
        return dice_mean, hd_mean, iou_mean, acc_mean, se_mean, sp_mean, mcc_mean, dices_std, hd_std, iou_std, acc_std, se_std, sp_std, mcc_std
